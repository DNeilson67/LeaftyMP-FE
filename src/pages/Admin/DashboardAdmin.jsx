        ...
        <>
            ...
            <div className="grid grid-flow-row lg:grid-flow-col gap-4">
                <motion.div initial={{ opacity: 0, y: 20 }}
                    ...
                    <StatsContainer label="Today's Production" value={statistics.sum_flour} unit="Kg" description="" color={"#C0CD30"} modal = {false}/>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                    transition={{ duration: 0.35, delay: 1.25 }}>
                    <StatsContainer label="Wet Leaves Collected" value={statistics.sum_wet_leaves} unit="Kg" description="" color={"#79B2B7"} modal = {false}/>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: -20 }}
                    transition={{ duration: 0.35, delay: 2 }}>
                    <StatsContainer label="Unscaled Pickups" value={unscaled} icon_unit={box} description="" color={"#0F7275"} modal = {false}/>
                </motion.div>
            </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }}
                ...
                {users.length !== 0 && <span className="text-xl font-bold">
                    Unapproved User
                </span>}
                {users.map((user) => (
                    <LongUser

    ...
    <div className="container mx-auto w-full">
      ...
      <div className="flex flex-wrap gap-4 justify-stretch">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.35, delay: 1 }} className="flex-grow flex-shrink lg:basis-1/5 basis-1/2">
          <StatsContainer label="Awaiting Dry Leaves" value={stats.awaiting || "0"} unit="Kg" color="#C0CD30" modal={false} frontIcon={AwaitingLeaves} />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.35, delay: 1.25 }} className="flex-grow flex-shrink lg:basis-1/5 basis-1/2">
          <StatsContainer label="In-Processed Leaves" value={stats.processed || "0"} unit="Kg" color="#79B2B7" modal={false} frontIcon={ProcessedLeaves} />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.35, delay: 1.5 }} className="flex-grow flex-shrink lg:basis-1/5 basis-1/2">
          <StatsContainer label="Expired Dry Leaves" value={stats.wasted || "0"} unit="Kg" color="#0F7275" modal={false} frontIcon={ExpiredDryLeaves} />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.35, delay: 1.75 }} className="flex-grow flex-shrink lg:basis-1/5 basis-1/2">
          <StatsContainer label="Total Collected Dry Leaves" value={stats.total || "0"} unit="Kg" color="#0F7275" modal={false} frontIcon={TotalDryLeaves} />
        </motion.div>
      </div>
      {selectedRowData && (
        <LeavesPopup status={selectedRowData.status} weight={selectedRowData.weight} centra_name={selectedRowData.name} expiredDate={selectedRowData.expiredDate} ref={leavesModalRef} dry_leaves={true} leavesid={selectedRowData.id} editable={false} />
      )}
    </div>
  );
};

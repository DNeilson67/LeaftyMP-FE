...
function MarketplaceShipmentOrders() {
    ...
    const fetchShipments = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await marketShipmentApi.getOrderShipments(0, 100);

            // Data is already filtered by status 'On Delivery' from the API
            setShipments(data);
            setHasLoaded(true);
        } catch (err) {
    ...
    const handleProcessOrder = async (shipment) => {
        ...
        setPopup({
            open: true,
            title: 'Warning',
            content: 'This is a demo. Order processing is disabled.',
            onConfirm: () => {
                setPopup(prev => ({ ...prev, open: false }));
                if (popupRef.current) popupRef.current.close();
            },

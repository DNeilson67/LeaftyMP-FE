            ...
            <div className='flex flex-row gap-4 mt-4'>
                {Buttons.map(({ label, image, disabled }, index) => (
                    <motion.button 
                        key={index}
                        disabled={disabled} 
                        className={`big-btn bg-gray-100 border-2 border-[#E8E8E8] p-6 rounded ${label === item ? 'active-btn' : ''} ${widthClass}`}
                    ...
                    >
                        <div className="flex flex-col items-center justify-between">
                            <motion.img 
                                src={image} 
                                alt={label} 
                                className={`${label === "Powder" ? 'w-1/4 mb-2':`w-${width} mb-1`}`}
                                whileHover={{ scale: 1.1 }} // Scale up on hover
                                transition={{ duration: 0.2 }}
                            />
                            <span className="text-center text-gray-700">{label}</span>
                        </div>
                    </motion.button>
                ))}
            </div>
